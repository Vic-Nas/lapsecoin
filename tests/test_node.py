...
class TestAbandonOnlyWhenItBuysSomething:
    """Abandoning a contested height buys one thing: a head start on the
    next one. That is worth having only if the next height is winnable.

    A node slower than the chain's own pace will be late for every height
    however early it starts, so the head start is a head start in a race it
    cannot finish, and the price is steep: a competitor finishing does not
    move our tip, so nothing was stopping the evaluation from completing.
    Finishing yields a real block with a real proof, which is this node's
    only evidence it did the work, and its only chance at the field having
    a bad round."""

    def _contested(self, node, window_left=0.5):
        node._draw_height = node.cs.height + 1
        node._draw_closes = time.monotonic() + window_left
        return [{"any": "candidate"}]

    def _chain_pace(self, node, monkeypatch, median):
        monkeypatch.setattr(block_mod, "block_time_stats",
                            lambda chain, h: {"own": median, "median": median,
                                              "diff": 0})

    def test_a_fast_node_still_abandons_a_lost_height(self, node_env, monkeypatch):
        node, *_ = node_env
        self._chain_pace(node, monkeypatch, 120.0)
        node._own_build_seconds.append(60.0)          # comfortably competitive
        assert node._should_abandon(node.cs, self._contested(node),
                                    time.monotonic()) is True

    def test_a_slow_node_finishes_instead(self, node_env, monkeypatch):
        node, *_ = node_env
        self._chain_pace(node, monkeypatch, 120.0)
        node._own_build_seconds.append(300.0)         # never going to be first
        assert node._should_abandon(node.cs, self._contested(node),
                                    time.monotonic()) is False

    def test_the_line_is_the_chain_pace_not_a_constant(self, node_env, monkeypatch):
        node, *_ = node_env
        node._own_build_seconds.append(100.0)
        self._chain_pace(node, monkeypatch, 120.0)    # we are faster than the field
        assert node._is_competitive(100.0) is True
        self._chain_pace(node, monkeypatch, 60.0)     # field sped up past us
        assert node._is_competitive(100.0) is False

    def test_too_little_chain_to_measure_counts_as_competitive(self, node_env, monkeypatch):
        node, *_ = node_env
        monkeypatch.setattr(block_mod, "block_time_stats", lambda chain, h: None)
        assert node._is_competitive(999.0) is True

    def test_an_uncontested_height_is_never_abandoned(self, node_env, monkeypatch):
        # Unchanged: with nobody else finished there is everything to gain
        # by carrying on, whatever our speed.
        node, *_ = node_env
        self._chain_pace(node, monkeypatch, 120.0)
        node._own_build_seconds.append(300.0)
        assert node._should_abandon(node.cs, [], time.monotonic()) is False
